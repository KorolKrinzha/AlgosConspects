...
class DijkstraAlgorithm {
    ...
    public int[] shortestPath(int source) {
        ...
        for (int i = 0; i < V - 1; i++) {
            // минимальные ребро от вершины до другой
...
На каждой итерации мы рассматриваем, как мы можем изменить минимальное расстояние от точки i до точки j с помощью ближайшей к i вершины.
    ...
    public static int[] dijkstra(int[][] graph, int startVertex) {
        ...
        while (!pq.isEmpty()) {
            ...
            for (int neighbor = 0; neighbor < n; neighbor++) {
                ...
                if (weight > 0
                        && !visited[neighbor] &&
                        currentDistance + weight < distances[neighbor]) {
                    distances[neighbor] = currentDistance + weight; // обновляем расстояние
                    pq.add(new Node(neighbor, currentDistance + weight)); // добавляем вершину в кучу
                }
            }
        }
...
```java
class SegmentTree {
    private int[] tree; // массив для хранения дерева отрезков
    private int[] array; // исходный массив

    public SegmentTree(int[] array) {
        this.array = array; // сохраняем исходный массив
        int n = array.length;
        // создаем пустое дерево отрезков
        int treeSize = 4 * n;
        tree = new int[treeSize];
        buildSegmentTree(1, 0, n - 1); // построение дерева отрезков
    }

    // Метод для построения дерева отрезков
    private void buildSegmentTree(int node, int start, int end) {
        // Если встретили листовой узел
        if (start == end) {
            // Листовой узел - значение из исходного массива
            tree[node] = array[start];
        } else {
            int mid = (start + end) / 2; // разбиваем текущий узел на два поддерева
            // Рекурсивно строим левое и правое поддерево
            buildSegmentTree(2 * node, start, mid);
            buildSegmentTree(2 * node + 1, mid + 1, end);
            // Объединяем значения поддеревьев в текущем узле
            // В данном случае задана сумма
            tree[node] = tree[2 * node] + tree[2 * node + 1];
        }
    }

    // Метод для выполнения операции суммы на отрезке [left, right]

    // Рекурсивный метод для печати дерева отрезков
    void printSegmentTree(int node, int start, int end) {
        if (start == end) {
            // Листовой узел - печатаем его значение
            System.out.println("Узел " + node + ": " + tree[node]);
        } else {
            int mid = (start + end) / 2;
            // Рекурсивно печатаем левое и правое поддерево
            printSegmentTree(2 * node, start, mid);
            printSegmentTree(2 * node + 1, mid + 1, end);
...
public class Main {
    public static void main(String[] args) {
        ...
        segmentTree.printSegmentTree(1, 0, array.length - 1);
    }
}
```
...
class SegmentTree {
    ...
    public int query(int left, int right) {
        return querySegmentTree(1, 0, array.length - 1, left, right);
    }

    // Рекурсивный метод для выполнения операции суммы на отрезке
    private int querySegmentTree(int node, int start, int end, int left, int right) {
        if (left > end || right < start) {
            // Отрезок полностью находится вне текущего узла
            return 0;
        } else if (left <= start && right >= end) {
            // Отрезок полностью содержит текущий узел
            return tree[node];
        } else {
            int mid = (start + end) / 2;
            // Рекурсивно выполняем операцию на левом и правом поддереве
            int leftSum = querySegmentTree(2 * node, start, mid, left, right);
            int rightSum = querySegmentTree(2 * node + 1, mid + 1, end, left, right);
            // Объединяем результаты поддеревьев
            return leftSum + rightSum;
        }
    }

    // Метод для обновления значения элемента в исходном массиве
    public void update(int index, int value) {
        updateSegmentTree(1, 0, array.length - 1, index, value);
    }

    // Рекурсивный метод для обновления значения элемента и перестроения дерева
    // отрезков
    private void updateSegmentTree(int node, int start, int end, int index, int value) {
        // проверка, что перед нами лист
        if (start == end) {
            // Достигли листового узла - обновляем значение
            array[index] = value;
            tree[node] = value;
            // если мы еще не достигли листового узла
            // то спускаемся ниже
        } else {
            int mid = (start + end) / 2;
            ...
            // После обновления поддеревьев
            // обновляем значение текущего узла в дереве отрезков
            tree[node] = tree[2 * node] + tree[2 * node + 1];
        }
    }
...
public class Main {
    public static void main(String[] args) {
        ...
        SegmentTree segmentTree = new SegmentTree(array);
        segmentTree.printArray();
        int sum = segmentTree.query(1, 4); // Сумма элементов от индекса 1 до 4
        System.out.println("Сумма элементов: " + sum);
        segmentTree.update(2, 6); // Обновление элемента с индексом 2 на значение 6
        sum = segmentTree.query(1, 4); // Сумма элементов от индекса 1 до 4 после обновления
        System.out.println("Сумма элементов после обновления: " + sum);
        segmentTree.printArray();
        segmentTree.printSegmentTree(1, 0, array.length - 1);
    }
}
...
public class Main {
    public static void main(String[] args) {
        ...
        SegmentTree segmentTree = new SegmentTree(array);
        segmentTree.printArray();
        int min = segmentTree.query(1, 4);
        System.out.println("Минимальный элемент: " + min);
        segmentTree.update(2, 6);
        min = segmentTree.query(1, 4);
        System.out.println("Минимальный элемент после обновления: " + min);
        segmentTree.printArray();
        segmentTree.printSegmentTree(1, 0, array.length - 1);
    }
}
...
public Node findPredecessor(Node node) {
    if (node.left != null) {
        ...
        while (parent != null && node == parent.left) {
            // если значение ключа текущего узла равно заданному ключу (то, которое в задаче)
            // то нам нужно найти предыдущий узел относительно этого ключа
